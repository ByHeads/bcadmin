...
try {
    ...
    if (-not $asset) {
        $asset = $assets | Where-Object { $_.name -match '\.msi$' } | Select-Object -First 1
    }

    if (-not $asset) {
        $assetNames = if ($assets.Count -gt 0) { ($assets | ForEach-Object { $_.name }) -join ', ' } else { '<none>' }
        throw "Could not find a Windows installer in release $version. Assets: $assetNames"
    }

    $downloadUrl = $asset.browser_download_url
    $fileName = if ($asset.name) { $asset.name } else { 'bcadmin-installer.exe' }
    $tmpFile = Join-Path $env:TEMP $fileName

    Write-Step "Pulling the $appName installer from GitHub ($fileName)"
    Invoke-WebRequest -Uri $downloadUrl -OutFile $tmpFile -UseBasicParsing
    Write-Done

    Write-Step "Running the installer"
    if ($tmpFile -match '\.msi$') {
        $installer = Start-Process -FilePath "msiexec.exe" -ArgumentList @('/i', $tmpFile) -Wait -PassThru
    }
    else {
